  ...
  reBuild(res: TableRes) {
    ...
    for (let i = 0; i < res.tableList.length; i++) {
      ...
      if (res.tableList[i].tableStatus == "已預約") {
        ...
        ctx.strokeStyle = 'red';
        str = res.tableList[i].tableId + res.tableList[i].tableStatus + res.tableList[i].tableCapacity;
      ...
      } else if (res.tableList[i].tableStatus == "可預約" || res.tableList[i].tableStatus == "空位") {
        ctx.strokeStyle = 'black';
